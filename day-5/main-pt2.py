    ...
    for line in file:
        ...
        if int(pt1[0]) == int(pt2[0]) and int(pt1[1]) != int(pt2[1]):
            ...
            for y in range(int(pt1[1]), int(pt2[1]) + s, s * 1):
                cmap[int(pt1[0])][y] +=1
        elif int(pt1[1]) == int(pt2[1]) and int(pt1[0]) != int(pt2[0]):
            s = np.sign(int(pt2[0]) - int(pt1[0]))
            for x in range(int(pt1[0]), int(pt2[0]) + s, s * 1):
                cmap[x][int(pt1[1])] +=1
        else:
            sx = np.sign(int(pt2[0]) - int(pt1[0]))
            sy = np.sign(int(pt2[1]) - int(pt1[1]))

            x = int(pt1[0])
            y = int(pt1[1])

            for i in range(abs(int(pt2[1]) - int(pt1[1])) + 1):
                cmap[x+i*sx][y+i*sy] +=1
